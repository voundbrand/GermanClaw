...
import {
  ...
  loadKanzleiAdminConfig,
  processDeterministicPipeline,
  resetKanzleiPipelineState,
  saveKanzleiAdminConfig,
  type KanzleiAdminConfig,
} from "./src/deterministic-pipeline.js";
...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    const resolveKanzleiStorePath = async () => {
      const rt = await ensureRuntime();
      return rt.manager.getStorePath();
    };
    const readPositiveInt = (value: unknown, fallback: number, max = 500) => {
    ...
    api.registerGatewayMethod(
      "voicecall.kanzlei.snapshot",
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        try {
          const storePath = await resolveKanzleiStorePath();
          const snapshot = await getKanzleiPipelineSnapshot(storePath, {
            callLimit: readPositiveInt(params?.callLimit, 50, 500),
            outboxLimit: readPositiveInt(params?.outboxLimit, 100, 500),
            deadLetterLimit: readPositiveInt(params?.deadLetterLimit, 50, 500),
            auditLimit: readPositiveInt(params?.auditLimit, 100, 500),
          });
    ...
    api.registerGatewayMethod(
      "voicecall.kanzlei.process",
      async ({ respond }: GatewayRequestHandlerOptions) => {
        try {
          const storePath = await resolveKanzleiStorePath();
          const processed = await processDeterministicPipeline(storePath);
    ...
    api.registerGatewayMethod(
      "voicecall.kanzlei.admin.get",
      async ({ respond }: GatewayRequestHandlerOptions) => {
        try {
          const storePath = await resolveKanzleiStorePath();
          const config = await loadKanzleiAdminConfig(storePath);
          respond(true, config);
        } catch (err) {
          sendError(respond, err);
        }
    ...
    api.registerGatewayMethod(
      "voicecall.kanzlei.admin.save",
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        try {
          if (!params || typeof params !== "object" || Array.isArray(params)) {
            respond(false, { error: "config object required" });
            return;
          }
          const storePath = await resolveKanzleiStorePath();
          const saved = await saveKanzleiAdminConfig(storePath, params as KanzleiAdminConfig);
          respond(true, saved);
        } catch (err) {
          sendError(respond, err);
        }
    ...
    api.registerGatewayMethod(
      "voicecall.kanzlei.reset",
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        try {
          const confirm = typeof params?.confirm === "string" ? params.confirm : "";
          if (confirm !== "reset") {
            respond(false, { error: "confirm=reset required" });
            return;
          }
          const storePath = await resolveKanzleiStorePath();
          await resetKanzleiPipelineState(storePath);
