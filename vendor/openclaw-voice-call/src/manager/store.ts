...
import { persistProjectedOutcome } from "../outcome-pipeline.js";
...
export function persistCallRecord(storePath: string, call: CallRecord): void {
  ...
  // Project deterministic terminal outcomes into a dedicated append-only stream.
  if (TerminalStates.has(call.state)) {
    void persistProjectedOutcome(storePath, call).catch((err) => {
      console.error("[voice-call] Failed to persist projected outcome:", err);
    });
  }
}
