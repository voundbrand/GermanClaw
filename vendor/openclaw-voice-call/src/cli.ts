...
function resolveDefaultOutcomePath(config: VoiceCallConfig): string {
  return path.join(path.dirname(resolveDefaultStorePath(config)), "outcomes.jsonl");
}

function resolveDefaultStoreDir(config: VoiceCallConfig): string {
  return path.dirname(resolveDefaultStorePath(config));
}
...
export function registerVoiceCallCli(params: {
  ...
  root
    .command("outcomes")
    .description("Summarize deterministic terminal intake outcomes")
    .option("--file <path>", "Path to outcomes.jsonl", resolveDefaultOutcomePath(config))
    .option("--last <n>", "Analyze last N records", "200")
    .action(async (options: { file: string; last?: string }) => {
      const file = options.file;
      const last = Math.max(1, Number(options.last ?? 200));
      ...
      if (!fs.existsSync(file)) {
        throw new Error("No outcome log file at " + file);
      }
      ...
      for (const line of lines) {
        try {
          const parsed = JSON.parse(line) as {
            ...
            dispatchReady?: unknown;
          };

          const category = String(parsed.outcomeCategory ?? "unknown");
          const area = String(parsed.practiceArea ?? "unknown");
          const urgency = String(parsed.urgency ?? "unknown");
          byCategory.set(category, (byCategory.get(category) ?? 0) + 1);
          byArea.set(area, (byArea.get(area) ?? 0) + 1);
          byUrgency.set(urgency, (byUrgency.get(urgency) ?? 0) + 1);
          if (parsed.callbackRequested === true) {
            callbackRequested += 1;
          }
          if (parsed.dispatchReady === true) {
            dispatchReady += 1;
          }
        } catch {
          // ignore malformed JSON lines
        ...
        JSON.stringify(
          {
            ...
            callbackRequested,
            dispatchReady,
            byOutcomeCategory: Object.fromEntries(byCategory),
            byPracticeArea: Object.fromEntries(byArea),
            byUrgency: Object.fromEntries(byUrgency),
          },
          null,
          2,
        ),
  ...
  root
    .command("kanzlei-pipeline")
    .description("Show deterministic Kanzlei outcome pipeline snapshot")
    .option("--store <path>", "Path to voice-call store directory", resolveDefaultStoreDir(config))
    .action(async (options: { store: string }) => {
      const summary = await summarizeKanzleiPipelineState(options.store);
      // eslint-disable-next-line no-console
      console.log(JSON.stringify(summary, null, 2));
    });

  root
    .command("kanzlei-pipeline-process")
    .description("Process deterministic Kanzlei outbox queue (retry/DLQ cycle)")
    .option("--store <path>", "Path to voice-call store directory", resolveDefaultStoreDir(config))
    .action(async (options: { store: string }) => {
      const result = await processDeterministicPipeline(options.store);
      // eslint-disable-next-line no-console
      console.log(JSON.stringify(result, null, 2));
    });
